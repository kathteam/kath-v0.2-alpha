...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python revel.py <dataset_path> <revel_db_path>")
        print("Example: python revel.py data.csv /reference_files/revel_with_transcript_ids.db")
        sys.exit(1)

    dataset_path = sys.argv[1]
    revel_db_path = sys.argv[2]
    result = main_revel_pipeline(dataset_path, revel_db_path)
    print(f"Processed {len(result)} variants successfully")
