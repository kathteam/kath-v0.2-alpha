"""
REVEL database initialization script.

Converts REVEL CSV file (6GB) into a SQLite database for efficient querying.
"""
...
class RevelTable:
    ...
    def init_table(file_path: str, db_path: str) -> None:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            # Chunk size
            chunk_size = 100_000
            chunk: List[Tuple[str, int, str, str, float]] = []
            ...
            with tqdm(desc="Processing rows", unit="Rows", unit_scale=True) as pbar:
                with open(file_path, "r") as f:
                    ...
                    for idx, row in enumerate(reader):
                        ...
                        try:
                            chunk.append(
                                (row["chr"], int(row["grch38_pos"]), row["ref"], row["alt"], float(row["REVEL"]))
                            )
                        except Exception as e:
                            logger.error(
                                f'Error in processing row (Possibly not supporting "X,Y ... " chromosome types): {e}'
                            )
                # Leftover chunk
                if chunk:
                    conn.execute("BEGIN TRANSACTION")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--overwrite", help="Overwrite the database file if it already exists", action="store_true"
    )
    parser.add_argument(
        "-l",
        "--log_level",
